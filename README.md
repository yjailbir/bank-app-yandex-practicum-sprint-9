...
На Windows помимо Helm можно запустить всё (от сборки до запуска сервисов) выполнением файла deploy.ps1.

Выключение программы и очистка ресурсов автоматически произойдёт при вызове shutdown.ps1.

Для запуска через Helm сначала нужно собрать образы вручную, проставив им тег 1.0